...
func init() {
	...
	user := flag.String("user", "lileibiao", "请输入 db 用户名\n")
	pass := flag.String("pass", "123456", "请输入 db 密码\n")
	name := flag.String("name", "clubdb", "请输入 db 名称\n")
	port := flag.String("port", "5432", "请输入 db 端口\n")
	table := flag.String("tables", "player_complaint_log", "请输入 table 名称，默认为“*”，多个可用“,”分割\n")
